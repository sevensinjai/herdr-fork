...
- **Notes** opens your editor on a notes file for the current session,
  `~/.config/herdr/sidecar/<session>.md`. Each named session (`herdr --session
  work`) has its own file, and `herdr session delete` leaves it in place.
- **Chat** runs a scratch agent, `claude` by default.

```toml
[sidecar]
width = "35%"
notes_command = '${EDITOR:-vi} "$HERDR_SIDECAR_NOTES"'
chat_command = "claude"
```
