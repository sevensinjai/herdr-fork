...
The Sidecar is a panel that slides in from the right edge over your panes. It
holds two terminals that keep running while it is hidden:

- **Notes** opens `nano` on a notes file for the current session, so you can
  type right away. Save with `ctrl+o`. Without `nano`, it uses `$EDITOR`, then
  `vi`. The file is `~/.config/herdr/sidecar/<session>.md`. Each named session
  (`herdr --session work`) has its own file, and `herdr session delete` leaves
  it in place.
- **Chat** runs a scratch agent, `claude` by default.

```toml
[sidecar]
width = "35%"
notes_command = 'command -v nano >/dev/null && exec nano "$HERDR_SIDECAR_NOTES"; exec ${EDITOR:-vi} "$HERDR_SIDECAR_NOTES"'
chat_command = "claude"
```

`width` accepts cells or a percentage of the pane area. The panel is at least 30
columns wide and always leaves 10 columns of panes visible. A command starts the
first time its tab is shown, in the focused pane's directory. If it exits, the
panel says so, and the next show starts it again.

`prefix+shift+s` shows or hides the Sidecar, and `prefix+shift+o` switches tabs.
Click a tab name to switch tabs, or click a pane to return focus to it. While
the Sidecar has focus, the prefix key still reaches Herdr.

To send text to the Sidecar, select it in any pane. Then right-click the pane
and choose `Send to Sidecar Notes` or `Send to Sidecar Chat`, or press
`prefix+shift+y` to send it to the tab in use. When the editor is not running,
the text is appended to the notes file. A Chat agent that is still starting
receives the text once it is ready for input. Scripts can do the same through
the `sidecar.send` socket method.
...
### Group the Agents panel

Set `group_by` to a custom pane metadata token to list agents under headings
instead of in one flat list:
...
Agents reporting the same value for that token are listed together under a
heading showing the value and how many agents it holds. Groups appear in the
order they are first encountered, and agents keep their order inside a group,
so the active sort still decides the ordering. Agents that have not reported
the token collect under a final `ungrouped` heading.

The token is a custom `$name` value reported by a plugin or script, the same
kind used in `rows`:
...
Grouping reorders the Agents panel itself, so Agent navigation and indexed
focus follow what you see: with `group_by` set, `focus_agent` numbering and
next/previous Agent walk the grouped order. Headings are labels only. They are
not selectable and take no mouse clicks. Omit `group_by`, or set it to an empty
string, to keep the flat list.

Grouping applies to a single server's Agents panel. The combined list shown
when several saved machines are connected keeps its own ordering.

List the stages you use in `group_values` to fix their heading order and offer
them in a menu:

```toml
[ui.sidebar.agents]
group_by = "$stage"
group_values = ["working", "verifying", "pending QA"]
```

Listed values come first, in the order given, followed by any other values in
use, then `ungrouped`. A listed value with no agents shows no heading.
Right-click an agent row to pick a stage, type a new one with `New stage…`, or
remove it with `Clear stage`. A stage typed in `New stage…` is also appended to
`group_values` in your config file, so it stays in the menu. The menu needs `group_by` set and a server that
accepts pane metadata reports from the client.
